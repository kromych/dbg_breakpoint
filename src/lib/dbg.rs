...
mod os {
    ...
    const KINFO_PROC_SIZE: usize = if cfg!(target_pointer_width = "64") {
        648
    } else {
        492
    };
    #[cfg(target_vendor = "apple")]
    const KINFO_PROC_FLAGS_OFFSET: u64 = if cfg!(target_pointer_width = "64") {
        32
    } else {
        16
    };
...
pub fn is_debugger_present() -> Option<DebuggerPresence> {
    if cfg!(miri) {
        None
    } else {
...
/// breakpoint instruction. This is nonetheless memory-safe, like [`std::process::abort`]
...
pub fn breakpoint_if_debugging() -> Option<DebuggerPresence> {
